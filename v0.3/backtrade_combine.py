...
def compute_profit(backtrade_path, backtrade_path_2, backtrade_path_3, kcb_set, base_shares=100, selected_topk=10):
    df = pd.read_csv(backtrade_path)
    df_2 = pd.read_csv(backtrade_path_2)
    df_3 = pd.read_csv(backtrade_path_3)
    count = 0
    profit = 0
    cost = 0
    win_count = 0
    ...
    df = pd.merge(df, df_2.drop(columns=[name for name in list(df_2) if name!='股票代码']), on=['股票代码'], how = 'inner')
    df = pd.merge(df, df_3.drop(columns=[name for name in list(df_3) if name!='股票代码']), on=['股票代码'], how = 'inner')
...
def backtrade_combine(start_day, end_day, kcb_set):
    total_profit = 0
    total_cost = 0
    total_win_count = 0
    total_count = 0
    for day in get_days(start_day, end_day):
        backtrade_path = f'backtrade/backtrade_{day}_{params.window_size}.csv'
        backtrade_path_2 = f'backtrade/backtrade_{day}_7.csv'
        backtrade_path_3 = f'backtrade/backtrade_{day}_14.csv'
        if not os.path.exists(backtrade_path):continue
        print(f'-----------------------{day}-----------------------')
        profit, cost, win_count, count = compute_profit(backtrade_path, backtrade_path_2, backtrade_path_3, kcb_set, selected_topk=params.selected_topk)
        total_profit += profit
        total_cost += cost
        total_win_count += win_count
        total_count += count
...
def main():
    ...
    if params.mode == 'train_and_backtrade':
        ...
        total_profit, total_cost, total_win_count, total_count = backtrade_combine(start_day, end_day, kcb_set)
