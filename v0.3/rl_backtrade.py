...
from utils import reduce_mem, get_today_date, get_delta_ago_date, auc_multi_class, find_top_k, get_days, date2int
...
def train_model(data, day, backtrade_path):
    ...
    feature_names = list(
        filter(lambda x: x not in [ycol, 'code', 'code_string', 'date', '', 'future_close'] and '日期' not in x and '板块名称' not in x, train.columns))
    ...
    kcb_set = set(kcb_spot['代码'].apply(lambda x:int(x[2:]))).union(set(cyb_spot['代码'].apply(lambda x:int(x[2:])))).union(set(st_spot['代码'].apply(lambda x:int(x[2:]))))

    latest_result = latest_result.drop([i for i,x in enumerate(latest_result['股票代码']) if int(x[3:]) in kcb_set]).reset_index(drop=True)
...
def compute_profit(backtrade_path, base_shares=100, selected_topk=10):
...
class params:
    ...
    # submit_path = f'submit/backtrade_{date_end}_{window_size}.csv'
