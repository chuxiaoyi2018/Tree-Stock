...
class Downloader(object):
    ...
    def run(self):
        header = ['date','time','code','open','high','low','close','preclose','volume','amount',
                 'adjustflag']
        if self.mode == "once":
            self.run_once()
        elif self.mode == "all":
...
#     mode = "all"
#     root_path = 'stockdata/2023_by_5min/'
#     mkdir(root_path)
...
#     for day in tqdm(get_days(date_start, date_end)):
#         sleep(20)
#         save_path = os.path.join(root_path, day + '.csv')
#         downloader = Downloader(root_path, date_start=day, date_end=day, mode=mode, frequency='5')
#         downloader.run()
...
#     print(f'Time : {end_time - begin_time}')
    ...
    # merge
    root_path = 'stockdata/2023_by_5min/'
    save_path = 'stockdata/d_data/data_5min.csv'
    df_list = []
