...
def fund_flow_feature(data):
    print('Begin fund flow feature')
    fund_flow_df = pd.read_csv(params.fund_flow_path)
    fund_flow_df['日期'] = fund_flow_df['日期'].apply(lambda x: int(x.replace('-', '')))
    fund_flow_df['code'] = fund_flow_df['code'].apply(lambda x: int(x[3:]))
    data = pd.merge(data, fund_flow_df, left_on=['date', 'code'], right_on=['日期', 'code'], how='left').reset_index(drop=True)
    return data
...
#     stock_industry.code = stock_industry.code.apply(lambda x:int(x[3:]))
    ...
    # concept/fundflow feature
    data = concept_feature(data)
    data = fund_flow_feature(data)
...
def train_preprocess(data, day):
    ...
    return X_train, Y_train, X_val, Y_val, test, feature_names

def train_model(data, day, backtrade_path):
    X_train, Y_train, X_val, Y_val, test, feature_names = train_preprocess(data, day)
    print('------------------Train Length------------------')
    ...
    latest_result = latest_result[latest_result.open<300].reset_index(drop=True)
    latest_result.to_csv(backtrade_path, index=False, encoding='utf_8_sig')
    
    # norepeat_result.to_csv(params.norepeat_window_size_submit_path, index=False, encoding='utf_8_sig')
    #result.to_csv(params.window_size_submit_path, index=False, encoding='utf_8_sig')
    
def get_kcb_set():
    kcb_spot = ak.stock_zh_kcb_spot()
    cyb_spot = ak.stock_cy_a_spot_em()
    st_spot = ak.stock_zh_a_st_em()
    cx_spot = ak.stock_zh_a_new()
    x_spot = ak.stock_zh_a_new_em()
    
    kcb_set = set()
    for _set in [kcb_spot, cyb_spot, st_spot, cx_spot, x_spot]:
        if '代码' in _set:kcb_set = kcb_set.union(set(_set['代码'].apply(lambda x:extract_digits(x))))
        elif 'code' in _set:kcb_set = kcb_set.union(set(_set['code'].apply(lambda x:extract_digits(x))))
...
class params:
    mode_list = ['debug', 'train_and_backtrade', 'backtrade']
    mode = mode_list[2]
    window_size = 3
    topk = 1000
    ...
    fund_flow_path = 'stockdata/fund_flow.csv'
