...
#     stock_industry.code = stock_industry.code.apply(lambda x:int(x[3:]))
    ...
    alpha_list += ['主力净流入-净额', '主力净流入-净占比', '超大单净流入-净额', '超大单净流入-净占比', '大单净流入-净额', '大单净流入-净占比', '中单净流入-净额', '中单净流入-净占比', '小单净流入-净额', '小单净流入-净占比']
    if params.mode != 'debug':
        data = reduce_mem(data, list(data))

    # alpha net
    length = params.history_length
...
def train_and_backtrade(start_day, end_day, kcb_set):
    ...
    for day in get_days(start_day, end_day):
        ...
        backtrade_path = f'backtrade/fund_backtrade_{day}_{params.window_size}.csv'
